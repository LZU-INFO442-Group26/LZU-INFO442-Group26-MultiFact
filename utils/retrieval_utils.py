...
def get_img_content_req(img_url):
    try:
        ...
        r = requests.get(img_url, headers=headers, stream=True,timeout=(5,5))
        ...
        if not ('http' in img_url or 'https' in img_url):
            try:
                ...
                im_pil = Image.open(requests.get(img_url2, headers=headers, stream=True,timeout=(5,5)).raw)
                im = np.asarray(im_pil)
                return im, im_pil
            except:
                try:
                    img_url3 = 'https:' + img_url
                    im_pil = Image.open(requests.get(img_url3, headers=headers, stream=True,timeout=(5,5)).raw)
                    im = np.asarray(im_pil)
                    return im, im_pil
                except:
                    return 0,0           
